...
public class TestPmsCoreApi_CostCenter extends TestPmsCoreApi
{
	...
	{	
		...
		.then()
			...
			.body("oid", is(oid))
	    	.body("any { it.key == '" + CostCenterDTO_.DTO_NAME_identification + "' }", is(true))
	    	.body("any { it.key == '" + CostCenterDTO_.DTO_NAME_description + "' }", is(true));
	}
	...
	public void t_113_GET_listAll_filters_shouldWork() throws Exception
	{
		...
		String identification = response1.path(CostCenterDTO_.DTO_NAME_identification);
		String description = response1.path(CostCenterDTO_.DTO_NAME_description);
		
		{
			String response = given()
				...
				.get(PATH_TO_RESOURCE).asString();
			long fetchedOid = from(response).getInt("[0]." + CostCenterDTO_.DTO_NAME_oid);
		...
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", CostCenterDTO_.DTO_NAME_identification + " eq '" + identification + "'")
				.get(PATH_TO_RESOURCE).asString();
			String fetchedIdentification = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_identification);
		...
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", CostCenterDTO_.DTO_NAME_description + " eq '" + description + "'")
				.get(PATH_TO_RESOURCE).asString();
			String fetchedDescription = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_description);
		...
		// Boolean AND expression
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", CostCenterDTO_.DTO_NAME_description + " eq '" + description
		        	+ "' and " + CostCenterDTO_.DTO_NAME_identification + " eq '" + identification + "'")
				.get(PATH_TO_RESOURCE).asString();
			String fetchedIdentification = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_identification);
			String fetchedDescription = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_description);
			assertThat(fetchedIdentification, equalTo(identification));
		...
		// Boolean OR expression
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", CostCenterDTO_.DTO_NAME_description + " eq '" + description
		        	+ "' or " + CostCenterDTO_.DTO_NAME_identification + " eq '" + identification + "'")
				.get(PATH_TO_RESOURCE).asString();
			String fetchedIdentification = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_identification);
			String fetchedDescription = from(response).getString("[0]." + CostCenterDTO_.DTO_NAME_description);
	...
	public void t_122_GET_listAll_orderByStringAsc_shouldWork() throws Exception
	{
		String response1 = given()
	        .spec(requestSpecBuilder.build())
	        .queryParam("orderBy", CostCenterDTO_.DTO_NAME_identification + " asc")
	        .get(PATH_TO_RESOURCE).asString();
		int count = from(response1).getList("").size();
		if (count < 3) return;
		
		String firstIdentification = from(response1).getString("[0]." + CostCenterDTO_.DTO_NAME_identification);
		String secondIdentification = from(response1).getString("[1]." + CostCenterDTO_.DTO_NAME_identification);
		String thirdIdentification = from(response1).getString("[2]." + CostCenterDTO_.DTO_NAME_identification);
	...
	public void t_122_GET_listAll_orderByMultiple_shouldWork() throws Exception
	{
		String response1 = given()
	        .spec(requestSpecBuilder.build())
	        .queryParam("orderby", CostCenterDTO_.DTO_NAME_identification + " asc," + CostCenterDTO_.DTO_NAME_description + " asc")
	        .get(PATH_TO_RESOURCE).asString();
		int count = from(response1).getList("").size();
		if (count < 3) return;
	...
	public void t_200_POST_newValidData_shouldReturnStatusCreatedWithLocation() throws Exception
	{
		...
		String jsonPayload = Json.createObjectBuilder()
		    .add(CostCenterDTO_.DTO_NAME_identification, domainKey)
		    .add(CostCenterDTO_.DTO_NAME_description, ENTITY_VALID_description)
		    .build()
		    .toString();
        ...
        .then()
			...
			.statusCode(HttpURLConnection.HTTP_OK)
			.contentType(ContentType.JSON)	        
	        .body(CostCenterDTO_.DTO_NAME_identification, is(domainKey))
	        .body(CostCenterDTO_.DTO_NAME_description, is(ENTITY_VALID_description));
	}
	...
	public void t_300_PUT_validData_shouldReturnStatusNoContent() throws Exception
	{
	    ...
	    int oldVersionNumber = oldResponse.path(CostCenterDTO_.DTO_NAME_versionNumber);
	    String oldDomainKey = oldResponse.path(CostCenterDTO_.DTO_NAME_identification);
		...
		String jsonPayload = Json.createObjectBuilder()
		    .add(CostCenterDTO_.DTO_NAME_oid, oid)
		    .add(CostCenterDTO_.DTO_NAME_identification, domainKey)
		    .add(CostCenterDTO_.DTO_NAME_description, newDescription)
		    .build()
		    .toString();
        ...
        .then()
			.statusCode(HttpURLConnection.HTTP_OK)
			.contentType(ContentType.JSON)
			.body(CostCenterDTO_.DTO_NAME_oid, is(oid))
			.body(CostCenterDTO_.DTO_NAME_versionNumber, greaterThan(oldVersionNumber))
	        .body(CostCenterDTO_.DTO_NAME_identification, is(oldDomainKey))
	        .body(CostCenterDTO_.DTO_NAME_description, is(newDescription));
	}
	...
	public void t_300_PUT_orginalData_shouldReturnStatusNoContentAndNoVersionChange() throws Exception
	{
	    ...
	    int oldVersionNumber = oldResponse.path(CostCenterDTO_.DTO_NAME_versionNumber);
	    String oldDomainKey = oldResponse.path(CostCenterDTO_.DTO_NAME_identification);
	    String oldDescription = oldResponse.path(CostCenterDTO_.DTO_NAME_description);
	    
	    JsonReader reader = Json.createReader(new StringReader(oldResponse.asString()));      
        JsonObject oldJsonObject = reader.readObject();       
        //oldJsonObject.put(CostCenterDTO_.DTO_NAME_description, "newDescription2");
        ...
        .then()
			...
			.contentType(ContentType.JSON)
			.body(CostCenterDTO_.DTO_NAME_oid, is(oid))
			.body(CostCenterDTO_.DTO_NAME_versionNumber, is(oldVersionNumber))
	        .body(CostCenterDTO_.DTO_NAME_identification, is(oldDomainKey))
	        .body(CostCenterDTO_.DTO_NAME_description, is(oldDescription));
	}
	...
	{	
	    ...
	    Response response = given()
	    	.spec(requestSpecBuilder.build())
	        	.body("{"
		        	+ "\"" + CostCenterDTO_.DTO_NAME_identification + "\":\"" + domainKey + "\","
		        	+ "\"" + CostCenterDTO_.DTO_NAME_description + "\":\"Description for " + domainKey + "\""
		        	+ "}")
	    		.post(PATH_TO_RESOURCE);
