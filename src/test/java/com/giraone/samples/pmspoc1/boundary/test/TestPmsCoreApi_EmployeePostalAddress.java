...
public class TestPmsCoreApi_EmployeePostalAddress extends TestPmsCoreApi
{
	...
	{		
		...
		Response oldResponse = given()
	        .spec(requestSpecBuilder.build())
	        .get(path);		
		int oid = oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_oid);
		String oldHouseNumber = oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_houseNumber);
		String newCity = "London";
		String newStreet = "Broadway";
				
		String jsonPayload = Json.createObjectBuilder()
		    .add(EmployeePostalAddressDTO_.DTO_NAME_oid, oid)
		    .add(EmployeePostalAddressDTO_.DTO_NAME_ranking, (int) oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_ranking))
		    .add(EmployeePostalAddressDTO_.DTO_NAME_countryCode, (String) oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_countryCode))
		    .add(EmployeePostalAddressDTO_.DTO_NAME_city, newCity)
		    //.add(EmployeePostalAddressDTO_.DTO_NAME_secondaryAddressLine, (String) oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_secondaryAddressLine))
		    .add(EmployeePostalAddressDTO_.DTO_NAME_street, newStreet)
		    .add(EmployeePostalAddressDTO_.DTO_NAME_houseNumber, (String) oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_houseNumber))
		    //.add(EmployeePostalAddressDTO_.DTO_NAME_poBoxNumber, (String) oldResponse.path(EmployeePostalAddressDTO_.DTO_NAME_poBoxNumber))
		    .build()
		    .toString();
		...
		given().spec(requestSpecBuilder.build()).when().get(path).then().statusCode(HttpURLConnection.HTTP_OK)
			...
			.body(EmployeePostalAddressDTO_.DTO_NAME_oid, is(oid))
			.body(EmployeePostalAddressDTO_.DTO_NAME_city, is(newCity))
			.body(EmployeePostalAddressDTO_.DTO_NAME_street, is(newStreet))
			.body(EmployeePostalAddressDTO_.DTO_NAME_houseNumber, is(oldHouseNumber));
	}
	...
	{				
		...
		String jsonPayload = Json.createObjectBuilder()
			...
			.add(EmployeePostalAddressDTO_.DTO_NAME_ranking, ranking)
			.build().toString();
		...
		.then()
			...
			.contentType(ContentType.JSON)
			.body(EmployeePostalAddressDTO_.DTO_NAME_city, is("city"))
			.body(EmployeePostalAddressDTO_.DTO_NAME_street, is("street"));
