...
public class TestPmsCoreApi_Employee extends TestPmsCoreApi
{
	...
	public void t_112_GET_listAll_skip1_shouldReturnNotFirst() throws Exception
	{
		String response1 = given()
	        ...
	        .queryParam("skip", 0)
	        .get(PATH_TO_RESOURCE).asString();		
		JsonPath jsonPath1 = from(response1);
		int count = jsonPath1.getList("blockItems").size();
		if (count < 3) return;
		
		long firstOid = jsonPath1.getLong("blockItems[0].oid");
		long secondOid = jsonPath1.getLong("blockItems[1].oid");
		...
		long firstOid2 = jsonPath2.getLong("blockItems[0].oid");
	...
	public void t_113_GET_listAll_filters_shouldWork() throws Exception
	{
		...
		String personnelNumber = response1.path(EmployeeDTO_.DTO_NAME_personnelNumber);
		String lastName = response1.path(EmployeeDTO_.DTO_NAME_lastName);
		//String dateOfBirth = response1.path(EmployeeDTO_.DTO_NAME_dateOfBirth);
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", CostCenterDTO_.DTO_NAME_oid + " eq " + oid)
				.get(PATH_TO_RESOURCE).asString();
		...
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", EmployeeDTO_.DTO_NAME_personnelNumber + " eq '" + personnelNumber + "'")
				.get(PATH_TO_RESOURCE).asString();
			long fetchedOid = from(response).getInt("blockItems[0].oid");
			assertThat(fetchedOid, equalTo(oid));
		}
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", EmployeeDTO_.DTO_NAME_oid + " eq " + oid + " and " + EmployeeDTO_.DTO_NAME_personnelNumber + " eq '" + personnelNumber + "'")
				.get(PATH_TO_RESOURCE).asString();
			long fetchedOid = from(response).getInt("blockItems[0].oid");
			assertThat(fetchedOid, equalTo(oid));
		}
		
		{
			String response = given()
		        .spec(requestSpecBuilder.build())
		        .queryParam("filter", EmployeeDTO_.DTO_NAME_lastName + " eq '" + lastName + "'")
				.get(PATH_TO_RESOURCE).asString();
			long fetchedOid = from(response).getInt("blockItems[0].oid");
			assertThat(fetchedOid, equalTo(oid));
		}
	...
	public void t_200_POST_newValidData_shouldReturnStatusCreatedWithLocation() throws Exception
	{
		...
		String jsonPayload = Json.createObjectBuilder()
			...
			.add(EmployeeDTO_.DTO_NAME_dateOfEntry, "2014-01-01T00:00:00.000Z")
			.add(EmployeeDTO_.DTO_NAME_nationalityCode, "DE")
			.add(EmployeeDTO_.DTO_NAME_countryOfBirth, "DE")
			.add(EmployeeDTO_.DTO_NAME_birthPlace, "Erlangen")
			.add(EmployeeDTO_.DTO_NAME_numberOfChildren, 2)
			.add(EmployeeDTO_.DTO_NAME_contactEmailAddress1, "jane.doe@mymail.com")
			.build().toString();
		...
		.then()
			...
			.statusCode(HttpURLConnection.HTTP_OK)
			.contentType(ContentType.JSON)
			.body(EmployeeDTO_.DTO_NAME_personnelNumber, is(ENTITY_VALID_domainKey))
			.body(EmployeeDTO_.DTO_NAME_costCenter + "." + CostCenterDTO_.DTO_NAME_oid, is((int) costCenterId)) // TODO: cast!
			.body(EmployeeDTO_.DTO_NAME_lastName, is("Doe"))
			//.body(EmployeeDTO_.DTO_NAME_dateOfBirth, is("1966-12-31T00:00:00.000Z")) // TODO: not working!
			.body(EmployeeDTO_.DTO_NAME_numberOfChildren, is(2))
			.body(EmployeeDTO_.DTO_NAME_contactEmailAddress1, is("jane.doe@mymail.com"));
	}
	...
	public void t_201_POST_newValidDataWithDateLong_shouldReturnStatusCreatedWithLocation() throws Exception
	{
		...
		String jsonPayload = Json.createObjectBuilder()
			.add(EmployeeDTO_.DTO_NAME_costCenter, Json.createObjectBuilder()
				.add(CostCenterDTO_.DTO_NAME_oid, costCenterId))
			.add(EmployeeDTO_.DTO_NAME_personnelNumber, ENTITY_VALID_domainKey)
			.add(EmployeeDTO_.DTO_NAME_lastName, "Date")
			.add(EmployeeDTO_.DTO_NAME_firstName, "John")
			.add(EmployeeDTO_.DTO_NAME_dateOfBirth, (new Date()).getTime() - 3600*24*365*40)
			.add(EmployeeDTO_.DTO_NAME_gender, EnumGender.M.toString())
			.add(EmployeeDTO_.DTO_NAME_dateOfEntry, (new Date()).getTime() - 3600*24*365*2)
			.add(EmployeeDTO_.DTO_NAME_nationalityCode, "DE")
			.add(EmployeeDTO_.DTO_NAME_countryOfBirth, "DE")
			.add(EmployeeDTO_.DTO_NAME_birthPlace, "Hof")
			.add(EmployeeDTO_.DTO_NAME_numberOfChildren, 2)
			.add(EmployeeDTO_.DTO_NAME_contactEmailAddress1, "john.date@mymail.com")
			.build().toString();
		...
		.then()
			...
			.body(EmployeeDTO_.DTO_NAME_personnelNumber, is(ENTITY_VALID_domainKey))
			.body(EmployeeDTO_.DTO_NAME_costCenter + "." + CostCenterDTO_.DTO_NAME_oid, is((int) costCenterId)) // TODO: cast!
			.body(EmployeeDTO_.DTO_NAME_lastName, is("Date"))
			.body(EmployeeDTO_.DTO_NAME_numberOfChildren, is(2))
			.body(EmployeeDTO_.DTO_NAME_contactEmailAddress1, is("john.date@mymail.com"));
	}
	...
	{		
		...
		String jsonPayload = Json.createObjectBuilder()
			...
			.add(EmployeeDTO_.DTO_NAME_postalAddresses, Json.createArrayBuilder().add(Json.createObjectBuilder()		
				.add(EmployeePostalAddressDTO_.DTO_NAME_city, "city")
				.add(EmployeePostalAddressDTO_.DTO_NAME_countryCode, "DE")
				.add(EmployeePostalAddressDTO_.DTO_NAME_houseNumber, "1")
				.add(EmployeePostalAddressDTO_.DTO_NAME_postalCode, "12345")
				.add(EmployeePostalAddressDTO_.DTO_NAME_street, "street")
		...
		.then()
			...
			.body(EmployeeDTO_.DTO_NAME_numberOfChildren, is(2))
			.body(EmployeeDTO_.DTO_NAME_contactEmailAddress1, is("john.date@mymail.com"))
			.body(EmployeeDTO_.DTO_NAME_postalAddresses + "." + EmployeePostalAddressDTO_.DTO_NAME_ranking, is(1))
			.body(EmployeeDTO_.DTO_NAME_postalAddresses + "." + EmployeePostalAddressDTO_.DTO_NAME_street, is("street"));
	}
	...
	public void t_300_PUT_validData_shouldReturnStatusNoContent() throws Exception
	{
		...
		.then()
			.statusCode(HttpURLConnection.HTTP_OK)
			.contentType(ContentType.JSON).body(EmployeeDTO_.DTO_NAME_oid, is(oid))
			.body(EmployeeDTO_.DTO_NAME_versionNumber, greaterThan(oldVersionNumber))
			.body(EmployeeDTO_.DTO_NAME_personnelNumber, is(oldPersonnelNumber))
			.body(EmployeeDTO_.DTO_NAME_lastName, is(newLastName))
			.body(EmployeeDTO_.DTO_NAME_numberOfChildren, is(newNumberOfChildren));
	}
	...
	public void t_300_PUT_costCenterChange_shouldReturnStatusNoContent() throws Exception
	{
		...
		String jsonPayload1 = Json.createObjectBuilder()
			.add(EmployeeDTO_.DTO_NAME_costCenter, Json.createObjectBuilder()
				.add(CostCenterDTO_.DTO_NAME_oid, c1))
			.add(EmployeeDTO_.DTO_NAME_personnelNumber, domainKey)
			.add(EmployeeDTO_.DTO_NAME_lastName, "Date")
			.add(EmployeeDTO_.DTO_NAME_firstName, "John")
			.add(EmployeeDTO_.DTO_NAME_dateOfBirth, (new Date()).getTime() - 3600*24*365*40)
			.add(EmployeeDTO_.DTO_NAME_gender, EnumGender.M.toString())
	...
	long createFreshEntityAndReturnOid(String domainKey)
	{
		...
		String jsonPayload = Json.createObjectBuilder()
			.add(EmployeeDTO_.DTO_NAME_personnelNumber, domainKey)
			.add(EmployeeDTO_.DTO_NAME_lastName, "LastName-" + domainKey)
			.add(EmployeeDTO_.DTO_NAME_firstName, "FirstName-" + domainKey)
			.add(EmployeeDTO_.DTO_NAME_dateOfBirth, "1966-12-31T00:00:00.000Z")
			.add(EmployeeDTO_.DTO_NAME_gender, EnumGender.F.toString())
			.add(EmployeeDTO_.DTO_NAME_dateOfEntry, "2014-01-01T00:00:00.000Z")
			.add(EmployeeDTO_.DTO_NAME_nationalityCode, "DE")
